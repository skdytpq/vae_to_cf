...
class SelfAttention(nn.Module): # hidden  , attention mask
    def __init__(self, args):
        ...
        self.attention_head_size = int(args.hidden_size / args.num_attention_heads) # attention divide => Defualt 32
    ...
    def transpose_for_scores(self, x):
        # x = batch X seq_len X 128
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size) # (128) (4,32)
        x = x.view(*new_x_shape)  #  (128 , 4 , 32)
        return x.permute(0, 2, 1, 3) #   Batch , 4 , seq_len , 32

    def forward(self, input_tensor, attention_mask):
        mixed_query_layer = self.query(input_tensor)  # Batch X 128
        mixed_key_layer = self.key(input_tensor)  # Batch X 128
        mixed_value_layer = self.value(input_tensor) # Batch X seq_len X 128
        query_layer = self.transpose_for_scores(mixed_query_layer) # Batch , 4 , seq_len , 32
        key_layer = self.transpose_for_scores(mixed_key_layer) # Batch , 4 , seq_len , 32
        value_layer = self.transpose_for_scores(mixed_value_layer) # Batch , 4 , seq_len , 32

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        # 
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)  # batch X 4 X seq_len X seq_len
