...
class SeqDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq_set = set(items)
        for _ in input_ids:
            target_neg.append(neg_sample(seq_set, self.args.item_size)) # 들어있지 않은 아이템 찾기
        # 각 id 에 들어가있지 않은 item 찾기
        if self.args.latent_data_augmentation or self.args.VAandDA: # random 하게 augmentation 진행
            dice = random.sample(range(3), k=1)
            copy_input_ids = copy.deepcopy(input_ids) # 아이템 전체 목록 copy
            if dice == 0:
                aug_input_ids = self.item_crop(copy_input_ids)
            elif dice ==1:
                aug_input_ids = self.item_mask(copy_input_ids)
            else:
                aug_input_ids = self.item_reorder(copy_input_ids) # 어규먼트 된 아이템 인풋
        ...
        input_ids = input_ids[-self.max_len:] # 패딩시킨 전체 dataset
    ...
    def item_crop(self, item_seq, eta=0.6):  # item_Seq: [batch, max_seq]
        ...
        num_left = math.floor(item_seq_len * eta) # eta 비율만큼 자른 후 길이
        crop_begin = random.randint(0, item_seq_len - num_left) # crop 시작할 정수 뽑기
        ...
        # 전체 아이템 시퀀스에서 eta 비율 만큼의 아이템을 자른다.
    ...
    def item_mask(self, item_seq,  gamma=0.3):
        ...
        return list(masked_item_seq) # 인덱스에 마스킹
